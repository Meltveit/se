...
const BusinessDetails: React.FC<BusinessDetailsProps> = ({ business }) => {
  // Format year founded
  const yearFoundedText = business.yearFounded 
    ? `Founded in ${business.yearFounded}` 
    : null;
  
  // Format employee count
  const employeeText = business.employeeCount 
    ? `${business.employeeCount} employees` 
    : null;
  
  // Combine the two if both exist
  const companyStats = [yearFoundedText, employeeText]
    .filter(Boolean)
    .join(' • ');
      ...
      <div className="space-y-6">
        ...
        {companyStats && (
          <div className="pt-4 border-t border-gray-200">
            ...
            <p className="mt-1 text-sm text-gray-500">{companyStats}</p>
          </div>
        )}
